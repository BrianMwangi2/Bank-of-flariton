...
// implimenting the use state where one is for the main and the other one is for updating the main function 
function TransactionsList({ transactions, searchTerm }) {
  ...
  const handleDelete = async (id) => {
    try {
      // Delete function to the API
      await fetch(`http://localhost:8001/transactions/${id}`, {
        method: "DELETE",
      });
      // Remove transanction deleted from the list 
  ...
  // Function that facillitate sorting by category 
  const handleSortByCategory = () => {
    setSortCriteria("category");
  };
  // Function that facillitate sorting by description 
